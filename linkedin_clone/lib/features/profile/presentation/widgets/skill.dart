...
class _SkillWidgetState extends State<SkillWidget> {
  ...
  Future<void> _checkEndorsementStatus() async {
    ...
    try {
      final hasEndorsed = await profileProvider.hasEndorsedSkill(widget.skill.skillName);
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to check endorsement status: $e')),
        );
      }
    }
  }
  ...
  Future<void> _toggleEndorsement() async {
    ...
    try {
      if (_hasEndorsed) {
        // await profileProvider.removeEndorsement(skillName);
      } else {
        // await profileProvider.addEndorsement(skillName);
      }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (widget.skill.position != null && widget.skill.position!.isNotEmpty)
                  Padding(
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => EndorsementsListPage(
