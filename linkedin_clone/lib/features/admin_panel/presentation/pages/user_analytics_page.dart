  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          "User Analytics",
          key: ValueKey("user_analytics_appbar"),
        ),
      ),
      body:
          user == null
              ? const Center(
                child: Text(
                  ...
                  key: ValueKey("no_user_analytics_text"),
                ),
              )
              : SingleChildScrollView(
                padding: const EdgeInsets.all(16),
                child: Column(
                  ...
                  children: [
                    _buildStatCard(
                      ...
                      value: user.totalUsers.toString(),
                      color: Colors.blue,
                      key: const ValueKey("stat_total_users"),
                    ),
                    const SizedBox(height: 16),
                    if (_mostReportedProfile != null)
                      Card(
                        ...
                        child: ListTile(
                          key: const ValueKey("most_reported_user_tile"),
                          leading:
                              _mostReportedProfile!['profilePicture']!
                                      .isNotEmpty
                                  ? CircleAvatar(
                      ...
                      _buildStatCard(
                        ...
                        color: Colors.redAccent,
                        subtitle: "Reported ${user.userReportedCount} times",
                        key: const ValueKey("stat_most_reported_user"),
                      ),
                    const SizedBox(height: 24),
                    const Text(
                      "Most Active Users",
                      ...
                      key: ValueKey("most_active_users_title"),
                    ),
                    const SizedBox(height: 10),
                    if (user.mostActiveUsers.isNotEmpty)
                      ...user.mostActiveUsers.map((u) {
                        ...
                        return Card(
                          margin: const EdgeInsets.symmetric(vertical: 6),
                          child: ListTile(
                            key: ValueKey("active_user_tile_${u.userId}"),
                            leading:
                                profilePicture.isNotEmpty
                                    ? CircleAvatar(
                                      backgroundImage: NetworkImage(
                      ...
                      const Text(
                        ...
                        key: ValueKey("no_active_users_text"),
                      ),
                  ],
                ),
              ),
