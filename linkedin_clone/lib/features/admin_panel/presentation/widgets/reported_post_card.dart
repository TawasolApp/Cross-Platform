...
class ReportedPostCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      key: ValueKey('reportedPostCard_${post.id}'),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              CircleAvatar(
                key: ValueKey('authorAvatar_${post.id}'),
                backgroundImage:
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      post.postAuthor,
                      key: ValueKey('authorName_${post.id}'),
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                    Text(
                      post.postAuthorRole,
                      key: ValueKey('authorRole_${post.id}'),
                      style: const TextStyle(fontSize: 12, color: Colors.grey),
                    ),
                    const SizedBox(height: 6),
                    Text(
                      post.postContent,
                      key: ValueKey('postContent_${post.id}'),
                    ),
                    ...
                    if (post.postMedia != null)
                      ClipRRect(
                        ...
                        child: Image.network(
                          ...
                          height: 140,
                          fit: BoxFit.cover,
                          key: ValueKey('postMedia_${post.id}'),
                        ),
                      ),
                  ],
                ),
          ...
          Column(
            ...
            children: [
              ...
              Row(
                children: [
                  CircleAvatar(
                    key: ValueKey('reporterAvatar_${post.id}'),
                    backgroundImage:
                  ...
                  Text(
                    post.reportedBy,
                    key: ValueKey('reporterName_${post.id}'),
              ...
              Text(
                "Reason: ${post.reason}",
                key: ValueKey('reportReason_${post.id}'),
              ),
              Text(
                "Reported at ${_formatDateTime(post.reportedAt)}",
                key: ValueKey('reportTime_${post.id}'),
              ...
              children: [
                Expanded(
                  child: ElevatedButton(
                    key: ValueKey('deletePostBtn_${post.id}'),
                    onPressed: onDelete,
                ...
                Expanded(
                  child: OutlinedButton(
                    key: ValueKey('ignorePostBtn_${post.id}'),
                    onPressed: onIgnore,
                ...
                Expanded(
                  child: ElevatedButton(
                    key: ValueKey('suspendUserBtn_${post.id}'),
  ...
  Widget _buildStatusBadge(String status) {
    ...
    return Container(
      key: ValueKey('statusBadge_$status'),
