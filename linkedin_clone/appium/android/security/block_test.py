from appium import webdriver
from appium.options.android import UiAutomator2Options
from appium.webdriver.common.appiumby import AppiumBy
import time
import sys
import os

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
from capabilities import get_capabilities
import utils

def test_login():

    user = utils.get_user("userNatalia")

    options = get_capabilities()

    driver = webdriver.Remote(
        command_executor="http://localhost:4723",
        options=options
    )

    time.sleep(5)

    # click on login button
    login_button = driver.find_element(by="accessibility id", value="Already on LinkedIn? Sign in")
    login_button.click()

    time.sleep(5)

    # enter email and password
    email_textbox = driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().className("android.widget.EditText").instance(0)')
    email_textbox.click()
    email_textbox.send_keys(user["email"])
    driver.hide_keyboard()

    password_textbox = driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().className("android.widget.EditText").instance(1)')
    password_textbox.click()
    password_textbox.send_keys(user["password"])
    driver.hide_keyboard()

    form_button = driver.find_element(AppiumBy.XPATH, '//android.widget.Button[@content-desc="Sign in"]')
    form_button.click()

    time.sleep(5)

    # assert logged in
    assert driver.find_element(by="accessibility id", value="Home\nTab 1 of 6").get_attribute("displayed") == 'true'
    assert driver.find_element(by="accessibility id", value="Settings\nTab 6 of 6").get_attribute("displayed") == 'true'

    driver.find_element(by="accessibility id", value="My Network\nTab 2 of 6").click()
    time.sleep(5)

    # search for user Virginie
    driver.find_element(by="accessibility id", value="Search").click()
    time.sleep(5)
    search_button = driver.find_element(AppiumBy.CLASS_NAME, 'android.widget.EditText')
    search_button.click()
    time.sleep(5)
    search_button.send_keys("Virginie")
    driver.hide_keyboard()
    time.sleep(5)
    driver.find_element(by="accessibility id", value="Show all Results").click()
    time.sleep(5)

    # block user Virginie
    driver.find_element(by="accessibility id", value="Virginie Gibson\nGlobal Division Representative").click()
    time.sleep(10)
    driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().className("android.widget.Button").instance(3)').click()
    time.sleep(5)
    driver.find_element(by="accessibility id", value="Block").click()
    time.sleep(5)
    driver.find_element(by="accessibility id", value="BLOCK").click()
    time.sleep(3)
    assert driver.find_element(by="accessibility id", value="User Blocked successfully").get_attribute("displayed") == 'true'
    time.sleep(5)
    driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, 'new UiSelector().className("android.widget.Button").instance(0)').click()
    time.sleep(5)

    # search for user Virginie again
    driver.find_element(by="accessibility id", value="Search").click()
    time.sleep(5)
    search_button = driver.find_element(AppiumBy.CLASS_NAME, 'android.widget.EditText')
    search_button.click()
    time.sleep(5)
    search_button.send_keys("Virginie")
    driver.hide_keyboard()
    time.sleep(5)
    driver.find_element(by="accessibility id", value="Show all Results").click()
    time.sleep(5)

    # assert user Virginie is not found
    try:
        driver.find_element(by="accessibility id", value="Virginie Gibson\nGlobal Division Representative")
        assert False, "User Virginie is still found"
    except Exception as e:
        assert True, "User Virginie is not found"

    driver.quit()
